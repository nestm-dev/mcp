...
- b4c4652: Expose report schemas, immutable types and JSON validation through the browser-safe `/report` subpath. Use standard UTF-8 encoding without Node globals while preserving report size and consistency checks.
...
- cb14f77: Added a bounded, lossy `tools/call` result projection. Text stays text under per-block and shared
  ...
  and every retained string is copied so a short projection cannot pin a hostile source string.
